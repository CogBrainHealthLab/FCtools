                   ...
                   dir.check=TRUE)
{
  ...
  for (sub in 1:NROW(sub.list))
  {
    ...
    if(((!file.exists(paste(output_dir,"/headmotion/",sub.list[sub],".csv",sep="")) & !file.exists(paste(headmotion_dir,"/FCmat/",sub.list[sub],".txt",sep=""))) & overwrite==F) | overwrite==T)
      {
      start=Sys.time()
      #filepaths
      ...
      write.table(headmotion.row, file=paste(output_dir,"/headmotion/",sub.list[sub],".txt",sep=""), row.names = F, col.names = F, sep=",",quotes=F)
