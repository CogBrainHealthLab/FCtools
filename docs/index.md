...
All the post-processing (e.g., `CIFTItoFC`) and data prep (e.g.,
`extract_links`) functions have been removed from this package. These
functions are now moved into a new R package called
[`MRIpostproc`](https://github.com/CogBrainHealthLab/MRIpostproc)
...
function can take input vectors of the following lengths:
...
[`vizConnectogram()`](https://cogbrainhealthlab.github.io/FCtools/reference/vizConnectogram.md)
function from the cloud, and run it on the `results` vector
...
![](reference/figures/FC_219.png)

**B. Customizing optional parameters**

The
[`vizConnectogram()`](https://cogbrainhealthlab.github.io/FCtools/reference/vizConnectogram.md)
allows you to customize the following visual parameters
...
![](reference/figures/FC_119_thinedges.png) This will be very effective
if the edges are nicely bundled up, which isn’t the case for the above.
So if you tried to reduce the thickness of the edge links and it doesn’t
help much, then perhaps you should use a chord diagram instead

**C. Multiple connectograms**

If necessary, you can plot out multiple connectograms and fit them
nicely in a grid format.
...
Here’s a basic use of the
[`vizChord()`](https://cogbrainhealthlab.github.io/FCtools/reference/vizChord.md)
function on an input vector of 7021 *non-zero values* ranging from -1 to
1.
...
![](reference/figures/FC_chord119.png)

Just like the
[`vizConnectogram()`](https://cogbrainhealthlab.github.io/FCtools/reference/vizConnectogram.md)
function, you can customize the `hot` and `cold` colors, as well as the
colors for the networks. Note that if your input vector has a length of
78 (derived from the 12x12 functional network matrices), you will need
to specify 12 colors (because 12 networks) for the `colorscheme`.
...
![](reference/figures/FC_chord12.png)

**C. Multiple connectograms**

Just as above, you can plot out multiple chord diagrams and fit them
nicely in a grid format.
...
![](reference/figures/heatmap219.png)
...
![](reference/figures/4xheatmap219.png)
